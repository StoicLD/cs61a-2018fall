...
class Player(object):
    ...
    def play(self, card_index):
        ...
        """
        "*** YOUR CODE HERE ***"
        #rand_index=random.randrange(len(self.hand))
...
class TutorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        "*** YOUR CODE HERE ***"
        for _ in range(3):
            #rand_index=random.randrange(len(opponent.hand))
            opponent.hand.pop(0)
        for _ in range(3):
            opponent.draw()
...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        #非常重要，可以避免出现index out或者x is not in list的情况
        #因为迭代的时候当你移除了一个元素，下标依旧递增，但是却会跳过一个元素
        ob=opponent.deck.cards[:]
        for j in ob:
            if(j.attack ==other_card.attack and j.defense ==other_card.defense):
