...
def is_bst(t):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(t, take_label, left_or_right):
        if(t.is_leaf()):
            return True
        else:
            if(len(t.branches) > 2):
                ...
                if(t.branches[1].label < t.label):
                    return False
                child_is_bst = True
                #左子树，不能超过自身label,自己是max_label
                child_is_bst = child_is_bst and helper(t.branches[0], t.label, 1)   #t.label 为最大值
                child_is_bst = child_is_bst and helper(t.branches[1], t.label, 0)  #t.label 为最小值
                return child_is_bst
            else:
                #只有一个子节点的情况
                child_is_bst = True
                if(left_or_right == 1):       #自身被判定为左子树
                    if(t.branches[0].label > take_label):
                        return False
                    return (child_is_bst and helper(t.branches[0], t.label, 1))
                elif(left_or_right == 0):
                    #自身被判定为右子树                    
                    if(t.branches[0].label < take_label):
                        return False
                    return (child_is_bst and helper(t.branches[0], t.label, 0))
                else:
                    return True
