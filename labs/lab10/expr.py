...
class LambdaExpr(Expr):
    ...
    def eval(self, env):
        #这里创建了一个lambdafunction的实例，因此parent frame是env
...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        """
        "*** YOUR CODE HERE ***"
        # if(self.operator.eval(env) not in env):
        #     raise NameError('the operand {} isn\'t exist',str(self.operator))
        # else:
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        d = zip(self.parameters, arguments)
        for i in d:
            child_env.update({i[0]:i[1]})
