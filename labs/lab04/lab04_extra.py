...
def put_piece(board, max_rows, column, player):
    ...
    for i in range(max_rows):
        if(board[max_rows-i-1][column]=='-'):
            board[max_rows-i-1] = replace_elem(board[max_rows-i-1],column,player)
...
def check_win(board, max_rows, max_cols, num_connect, row, col, player):
    ...
    if(check_win_row(board,max_rows,max_cols,num_connect,row,player)):
        return True
    #for j in range(max_cols):
    if(check_win_column(board,max_rows,max_cols,num_connect,col,player)):
        return True
    return False
