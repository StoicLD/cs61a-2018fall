...
#这个文件实现了paser的部分，根据读入的expression式子解析成为内置类型Pair
#首先输入是用tokenlize按行切割为token，然后放入一个Buffer类中，这个Buffer类在
#传入scheme_reader函数中，最终返回由Pair或者nil组成的内置类型，以便于我们下一步的解析
#   如(+ 1 1)解析为 Pair('+', Pair(1, Pair(1, nil)))
    ...
    try:
        if src.current() is None:
            ...
            if(final == ')'):
                ...
                raise SyntaxError('unexpected token: {0}'.format(next_expr))
            # END PROBLEM 2
        else:
            # BEGIN PROBLEM 1
            "*** YOUR CODE HERE ***"
